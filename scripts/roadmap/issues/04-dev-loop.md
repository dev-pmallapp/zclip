...
## Acceptance criteria
- [x] A `dev` layout KDL file loads zclip from `file:target/wasm32-wasip1/debug/zclip.wasm`
- [x] A task runner target (e.g. `just dev` or a shell script) builds the plugin and runs
      `zellij action start-or-reload-plugin file:target/wasm32-wasip1/debug/zclip.wasm`
- [x] A documented ad-hoc load path exists using `zellij plugin --floating file:target/wasm32-wasip1/debug/zclip.wasm -s`
- [x] Watching the crate for changes (e.g. via `cargo watch` or `entr`) triggers an automatic rebuild+reload
- [x] Instructions for the dev loop are written in the README or a `docs/dev.md` file
